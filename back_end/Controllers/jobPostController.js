...
module.exports = {
  getAllJobList: (req, res) => {
    connection.query("SELECT * from jobs", function (err, results) {
      ...
      return res.send({
        message: "Successfully received job list",
        data: results,
      });
    });
  },
  ...
  addJobPost: (req, res) => {
    ...
    const data = req.body;
    connection.query(
      `insert into Jobs (company_logo, company_name, job_role,location,
      ...
      (err, results) => {
        ...
        return res.send({
          message: "Successfully added new job item",
          data: results,
        });
      }
    );
  ...
  updateJobPost: (req, res) => {
    ...
    connection.query(
      `update jobs set company_logo ='${data.company_logo}',
      ...
      (err, results) => {
        if (err) {
          ...
          return res
            .status(400)
            .send({ message: "Error while updating data", error: err });
        }
        return res.send({
  ...
  deleteJob: (req, res) => {
    ...
    const id = req.params.jobId;
    connection.query(`delete from jobs where jobId =${id}`, (err, results) => {
      if (err) {
  ...
  getUserjobList: (req, res) => {
    ...
    connection.query(
      `SELECT * from jobs where userId = ${id} `,
